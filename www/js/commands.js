...
function init_command_panel() {
    TLN.append_line_numbers("commandEditor");
}
...
function Monitor_check_autoscroll() {
    if (preferenceslist[0].enable_autoscroll === 'true') Monitor_output_autoscrollcmd();
...
function Monitor_output_Update(message) {
    if (message) {
        ...
        //if ((message.startsWith("ok")) && curCmdStatus == 1) SendCommandLine();
    }
    var regex = /ok T:/g;
    ...
    var isverbosefilter = true; //document.getElementById("monitor_enable_verbose_mode").checked;
...
function SendCommandLine() {
    var cmds = document.getElementById("commandEditor").value.split("\n");
    if (curCmdLine < cmds.length && curCmdStatus == 0) curCmdStatus = 1;
    if (curCmdLine >= cmds.length && curCmdStatus == 1) {
        curCmdStatus = 0;
        curCmdLine = 0;
    } 
    if (curCmdStatus == 0) return;
    TLN.setSelectedLine(curCmdLine);
    var cmd = cmds[curCmdLine];
    curCmdLine++;
    var url = "/command?commandText=";
    cmd = cmd.trim();
    if (cmd.trim().length == 0) return;
