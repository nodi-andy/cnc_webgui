var TLN = {
  eventList: {},
  selectedLine: 0,
  setSelectedLine: function(selLine) {
    this.selectedLine = selLine;
    var curLine = document.getElementsByClassName("tln-active-line")[0];
    if (curLine) curLine.classList.remove("tln-active-line");
    curLine = document.getElementsByClassName("tln-line")[this.selectedLine];
    if (curLine) curLine.classList.add("tln-active-line");
  },
  update_line_numbers: function(ta, el) {
    // var's check if there are more or less lines than before
    var line_count = ta.value.split("\n").length;
    ...
    if(difference > 0) {
      ...
      while(difference > 0) {
        ...
        if (line_count - difference == this.selectedLine) line_number.classList.add("tln-active-line");
  ...
  append_line_numbers: function(id) {
    ...
    var __change_hdlr = function(ta, el) {
      return function(e) {
        ...
        || e.code=="NumpadEnter")
          ta.scrollLeft = 0;
        // Whether we scrolled or not, var's check for any line count updates
        TLN.update_line_numbers(ta, el);
      }
    }(ta, el);
  ...
  remove_line_numbers: function(id) {
    ...
    var __wrapper_chck = ta.previousSibling;
